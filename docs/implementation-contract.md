...
Set only `EAPOLKIT_BIND_ADDRESS` in `.env` to select a concrete LAN IP for opt-in trusted-LAN HTTP. Compose passes the same value to the app so its default allowed hosts include that IP plus `localhost`, `127.0.0.1`, and `::1`. A nonempty `EAPOLKIT_ALLOWED_HOSTS` remains an authoritative override. Wildcard bind addresses do not expand accepted HTTP hosts. Preserve host, same-origin, CSRF, HttpOnly, and SameSite checks.

Trusted-LAN HTTP does not encrypt passwords, session cookies, credentials, or private material in transit. State this limitation in deployment guidance and prohibit public-internet publication. Keep SSH tunnel and HTTPS reverse-proxy options; use `EAPOLKIT_SECURE_COOKIES=1` for HTTPS deployment. The kit does not provision host addresses, firewalls, certificates, or a proxy. Initial deployment happens in the Debian terminal; later browser access can come from a separate LAN client.
...
Fields: `id`, `name`, `host`, `port` (1812), `timeout_seconds` (30, range 5 to 120), `nas_identifier` (`eapol-test-kit`), optional `nas_ip_address`, `calling_station_id` (`02:00:00:00:00:01`), and `extra_attributes` (array of attribute rows described next). Attribute types are `string`, `integer`, `hex`, and `ipaddr`; validate them before encoding them for protected runtime input. `secret` is write-only and contains 1 to 4096 UTF-8 bytes. Reject NUL, and preserve every other byte, including CR/LF and leading or trailing whitespace. Do not trim or normalize it. The protected runtime input uses the same byte contract. Reads include `has_secret`. The target host supports IP literals and DNS names. NAS-IP-Address is an attribute, not packet source selection.
...
`POST /api/profiles/{id}/duplicate` accepts optional `{name, expected_outcome}` and returns a new profile. Preserve the encrypted saved password on the server without returning it to the browser. The new record has its own ID and references the same selected certificate assets. This explicit same-user operation avoids requiring password re-entry for ordinary copies. The operator edits the new record separately to change a password for a negative test.

Fields: `id`, `name`, `method`, `identity`, optional `anonymous_identity`, `ca_certificate_id`, optional `client_identity_id`, `server_name`, `tls_min_version` (`1.2` or `1.3`), `tls_max_version` (`auto`, `1.2`, or `1.3`), `fragment_size` (1398), `expected_outcome` (`accept`, `reject`, or `certificate_error`), and `allow_expired_client_certificate` (false). `password` is write-only; reads include `has_password`. A recipe can be saved before it is runnable; validate all required credentials/trust assets before execution. Require explicit server CA and expected server name. Do not add an insecure validation bypass in v1. Certificate expiry bypass applies only to an intentionally expired client certificate, not server trust.

`GET /api/profiles/{id}/preview` returns `{configuration, warnings}`. Redact secrets and use asset references instead of private filesystem paths. The UI supports profile duplication, including a negative-test copy with an explicit expected outcome. Editable advanced fields and an exact generated preview are the first version's configuration interface; do not execute arbitrary uploaded wpa configuration directives, plugins, engines, or filesystem paths.
