...
def test_http_attribute_privacy_preservation_and_scoped_keys(tmp_path):
    ...
    with TestClient(app) as client:
        _login(client)
        original = client.post("/api/profiles", json={"name":"Rows","method":"ttls-pap","extra_attributes":[{"id":27,"type":"integer","value":"00042"},{"id":1,"type":"string","value":hidden,"sensitivity":"private"},{"id":26,"type":"hex","value":"00000009010664656d6f","sensitivity":"public"}]} )
        ...
        stored = app.state.store.get("profile", target["id"])["extra_attributes"]
        ...
        endpoint = f"/api/profiles/{target['id']}"
        keep = [{key: row[key] for key in ("key", "id", "type")} for row in rows]
        changed = client.put(endpoint, json={"name":"Reordered","method":"ttls-pap","extra_attributes":list(reversed(keep))})
        ...
        unchanged = client.put(endpoint, json={"name":"List omitted","method":"ttls-pap"})
        require(unchanged.status_code == 200 and len(unchanged.json()["extra_attributes"]) == 3)
        second = client.post("/api/profiles", json={"name":"Other","method":"ttls-pap"}).json()
        cross = client.put(f"/api/profiles/{second['id']}", json={"name":"Other","method":"ttls-pap","extra_attributes":[keep[1]]})
        require(cross.status_code == 422)
        duplicate = client.put(endpoint, json={"name":"Duplicate","method":"ttls-pap","extra_attributes":[keep[0],keep[0]]})
        require(duplicate.status_code == 422)
        bad_identity = client.put(endpoint, json={"name":"Wrong encoding","method":"ttls-pap","extra_attributes":[{**keep[1],"type":"hex"}]})
        require(bad_identity.status_code == 422)
        disclose = client.put(endpoint, json={"name":"No replacement","method":"ttls-pap","extra_attributes":[{**keep[1],"sensitivity":"public"}]})
        require(disclose.status_code == 422)
        public = client.put(endpoint, json={"name":"Replacement","method":"ttls-pap","extra_attributes":[{**keep[1],"sensitivity":"public","value":replacement}]})
        ...
        cleared = client.put(endpoint, json={"name":"Cleared","method":"ttls-pap","extra_attributes":[]})
...
def test_http_opaque_and_known_credential_classification(tmp_path):
    ...
    with TestClient(create_app(settings)) as client:
        _login(client)
        denied = client.post("/api/profiles", json={"name":"Known","method":"ttls-pap","extra_attributes":[{"id":2,"type":"string","value":value,"sensitivity":"public"}]})
        ...
        response = client.post("/api/profiles", json={"name":"Opaque","method":"ttls-pap","extra_attributes":[{"id":26,"type":"hex","value":"00"}]})
        require(response.status_code == 200)
        target = response.json()
        row = target["extra_attributes"][0]
        require(row["sensitivity"] == "private" and "value" not in row)
        public = client.put(f"/api/profiles/{target['id']}", json={"name":"Public replacement","method":"ttls-pap","extra_attributes":[{"key":row["key"],"id":26,"type":"hex","sensitivity":"public","value":""}]})
        require(public.status_code == 200)
...
def test_http_startup_migration_preserves_public_legacy_bytes_and_keys(tmp_path):
    ...
    from eapolkit.models import ProfileInput
    row = ProfileInput(name="Legacy", method="ttls-pap").model_dump(exclude={"password"})
    row.update(id="legacy", has_password=False, extra_attributes=[{"id":26,"type":"hex","value":legacy_value}])
    store.put("profile", row)
    store.close()
    with TestClient(create_app(settings)) as client:
        password = _login(client)
        migrated = client.get("/api/profiles").json()[0]["extra_attributes"][0]
        require(migrated["value"] == legacy_value and migrated["sensitivity"] == "public")
        key = migrated["key"]
        changed = client.put("/api/profiles/legacy", json={"name":"Still public","method":"ttls-pap","extra_attributes":[{"key":key,"id":26,"type":"hex"}]})
        require(changed.status_code == 200)
    ...
    with TestClient(create_app(settings)) as client:
        ...
        persisted = client.get("/api/profiles").json()[0]["extra_attributes"][0]
