...
def public_target(store: Store, target: dict) -> dict:
    ...
    return {key: value for key, value in public(target).items() if key != "extra_attributes"}


def public_profile(store: Store, profile: dict) -> dict:
    """Return the ordinary profile shape shared by API responses and snapshots."""
    visible = {key: value for key, value in public(profile).items() if key not in {"extra_attributes", "expected_outcome"}}
    visible["extra_attributes"] = public_rows(store, profile.get("extra_attributes", []))
    return visible
...
def migrate_targets(store: Store) -> int:
    return migrate_records(store, "target")


def migrate_records(store: Store, kind: str) -> int:
    """Convert legacy plaintext extra-attribute rows before serving requests.
    ...
    with store.transaction():
        ...
        # Validate every record before the first write; keep other Store users serialized.
        for target in changes:
            store.put(kind, target)
    return len(changes)
...
def migrate_history(store: Store) -> int:
    ...
    The scan is independent of extra-attribute upgrades. Each run is written completely;
    ...
    for original in store.list("run"):
        ...
        values = []
        for kind in ("target", "profile"):
            display = snapshot.get(kind)
            rows = display.get("extra_attributes") if isinstance(display, dict) else None
            if not isinstance(rows, list):
                continue
            live_rows = []
            object_id = original.get(f"{kind}_id")
            if object_id and display.get("id", object_id) == object_id:
                try:
                    live_rows = _stored_rows(store.get(kind, object_id).get("extra_attributes", []))
            ...
            for row in rows:
                ...
                if sensitivity == "private":
                    ...
                    item.pop("_value", None)
                visible.append(item)
            record["snapshot"][kind]["extra_attributes"] = visible
        if values:
            ...
            for kind, fields in (("target", ("name", "host", "nas_identifier", "nas_ip_address")), ("profile", ("name", "identity", "anonymous_identity", "server_name", "calling_station_id"))):
